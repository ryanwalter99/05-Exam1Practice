...
def problem2a(circle, rectangle, window):
    ...
    x2 = rectangle.get_lower_left_corner()
    window.render(.05)
    window.continue_on_mouse_click()
    line = rg.Line(x,x2)
...
def problem2b(rect, n, delta, win):
    ...
    rect.attach_to(win)
    for _ in range(n-1):
        point1 = rg.Point(rect.get_upper_left_corner().x - delta,rect.get_upper_left_corner().y - delta)
        point2 = rg.Point(rect.get_lower_right_corner().x +delta, rect.get_lower_right_corner().y + delta)
